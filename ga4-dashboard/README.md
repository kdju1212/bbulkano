...
| 구글시트 대시보드 | — | 비공개 구글 시트(날짜/매체/캠페인/광고비/클릭/구매/매출 열)를 로그인한 사람 본인의 구글 계정 권한으로 읽기 전용 조회. 별도 서비스 계정·API 키 없이, GA4 로그인에 쓰는 것과 같은 구글 OAuth에 시트 읽기 권한만 추가로 얹었다 — 그 계정이 시트에 접근 권한이 없으면 구글이 자체적으로 막아준다. 총 광고비·매출·ROAS·CPC·CPA·CTR 카드, 날짜별 광고비·매출·ROAS 추이, 매체별 비교, 날짜/매체/캠페인 필터 지원. 저장/캐시 없이 매번 시트를 새로 읽어서 항상 최신값 |

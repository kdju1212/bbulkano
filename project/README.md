...
`main.py` 하나로 기존 `KG에듀원_임용고시_통합가공.xlsm` 템플릿과 `json_모음` 규칙, `재료` CSV 파일을 읽어 보고서 파일을 생성합니다.
...
재료 CSV 파일을 매번 `재료/` 폴더에 직접 넣지 않고, 브라우저에서 업로드해 결과 파일을 바로 받을 수 있습니다.
...
1. `001_임용고시_클리너.json`으로 대상 시트의 기존 데이터 영역을 비웁니다.
2. `002_임용고시_외부통합.json`의 범위 매핑을 그대로 읽어 CSV 데이터를 템플릿 시트에 입력합니다.
3. `조건` 폴더의 정제, 필다운, 미포함, 공백 0 처리 규칙을 순서대로 실행합니다.
4. `008_임용고시_내부통합.json`으로 RAW 시트 데이터를 `종합시트`에 통합합니다.
